...
export type ArktsParseFallbackStage = 'raw' | 'light' | 'brace' | 'opaque';

export interface ArktsParseFallbackResult {
  tree: Parser.Tree;
  content: string;
  stage: ArktsParseFallbackStage;
}

type ParseOptions = Parameters<Parser['parse']>[2];

const isIdent = (ch: string | undefined): boolean => !!ch && /[A-Za-z0-9_$]/.test(ch);
const isSpace = (ch: string | undefined): boolean =>
  ch === ' ' || ch === '\t' || ch === '\n' || ch === '\r';

function previousNonSpace(source: string, index: number): string | undefined {
...
function hasColonAfter(source: string, closeIndex: number): boolean {
  let quote: string | null = null;
  let escaped = false;
...
export function maskArktsTernaryParens(source: string): string {
...
export function maskArktsInvalidNumericLiterals(source: string): string {
  const chars = source.split('');
  for (let i = 0; i < source.length; i++) {
    const ch = source[i];
...
export function maskArktsAngleAssertions(source: string): string {
  const chars = source.split('');
...
export function maskArktsBraceBodies(source: string): string {
  const chars = source.split('');
  const stack: number[] = [];
  let quote: string | null = null;
  let escaped = false;
...
export function maskArktsOpaque(source: string): string {
  return source.replace(/[^\r\n]/g, ' ');
}

export function sanitizeArktsForParseFallback(
  source: string,
  stage: Exclude<ArktsParseFallbackStage, 'raw'>,
): string {
...
export function parseArktsWithFallback(
  parser: Parser,
  source: string,
  options?: ParseOptions,
): ArktsParseFallbackResult {
  const raw = parser.parse(source, undefined, options);
  ...
  if (lightContent !== source) {
    const light = parser.parse(lightContent, undefined, options);
  ...
  if (braceContent !== lightContent) {
    const brace = parser.parse(braceContent, undefined, options);
  ...
  const opaque = parser.parse(opaqueContent, undefined, options);
